/*
 ...
 Database:
 CREATE TABLE person_test (id INTEGER PRIMARY KEY, name VARCHAR(255));
 CREATE SEQUENCE person_seq START WITH 1 INCREMENT BY 1 NOMAXVALUE;
 CREATE TRIGGER person_pk_trigger BEFORE INSERT ON person_test FOR EACH row
 BEGIN
...
describe('oracle driver', function () {
  ...
  it("should select with single quote", function (done) {
    var self = this;
    self.connection.execute("INSERT INTO person_test (name) VALUES (:1)", ["Bill O'Neil"], function (err, results) {
      if (err) {
        console.error(err);
        return;
      }
      self.connection.execute("INSERT INTO person_test (name) VALUES (:1)", ["Bob Johnson"], function (err, results) {
        if (err) {
          console.error(err);
          return;
        }
        self.connection.execute("SELECT * FROM person_test WHERE name = :1", ["Bill O'Neil"], function (err, results) {
          if (err) {
            console.error(err);
            return;
          }
  ...
  it("should insert with returning value", function (done) {
    var self = this;
    self.connection.execute("INSERT INTO person_test (name) VALUES (:1) RETURNING id INTO :2",
  ...
  it("should support select with getColumnMetaData", function(done) {
    ...
    self.connection.execute("INSERT INTO person_test (name) VALUES (:1)", ["Bill O'Neil"], function(err, results) {
      ...
      self.connection.execute("SELECT * FROM person_test", [], {getColumnMetaData:true}, function(err, results) {
        if(err) { console.error(err); return; }
        assert.equal(results.length, 1);
